...
from python.Snowpatrol import constants1
...
class SnowpatrolDataModel:
    ...
    def get_active_licenses(self) -> pd.DataFrame:
        try:
            active_licenses = self._session.sql(f""" 
                                    with 
                                        combined_logs as (
                                        select app_id, session_user from {constants1.TBL_APP_LOGS}
                                        union 
                                        select app_id, session_user from {constants1.TBL_OKTA_USERS}
                                        ),
                                        logs_with_metadata as (
                                            select ma.app_name, cl.app_id, cl.session_user, em.title, em.department, em.division
                                            from {constants1.TBL_EMP_METADATA} em 
                                            join combined_logs cl on (em.session_user = cl.session_user)
                                            join {constants1.TBL_MONITORED_APPS} ma on (cl.app_id = ma.app_id)
                                        )
    ...
    def get_revocation_recommendations(self, app_id:int, run_id:str = None) -> pd.DataFrame:
        try:
            result = self._session.table(f"{constants1.TBL_LICENSE_REVOCATION_RECOMMENDATION}")\
