...
On the "Export Data" page, we retrieve data from Snowflake tables and transform it into dataframes. Users can filter data based on specific date ranges, as well as categorize it by application type.
 
Additionally, we provide the capability to view filtered logs for all users and those with revoked access. By clicking the "Download" button, users can easily save the filtered data as a CSV file to their local system.
