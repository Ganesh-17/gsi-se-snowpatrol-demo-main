...
def binarize_and_stuff_data(auth_logs, work_days, emp_metadata, whitelisted_users, cutoff_days, target_variable_no_login):

    # Filter emps not having any of the elevated job titles
    emp_metadata_1 = emp_metadata.filter(~F.call_udf(f"{SNOWPATROL_DB}.{DEVOPS_SH}.udf_contains_anyof", F.col("title"), F.lit(TITLES_KW_WITH_ACCESS_RETAINED)))
    ...
    # Filter authentication logs to drop ex-emps 
    offset_days = cutoff_days
    ex_emp_cutoff_date = today - timedelta(days = offset_days)

    # window1 = Window.partition_by("session_user")

    filtered_logs_3 = filtered_logs_2.join(emp_metadata_1, 
                                             on=(F.lower(filtered_logs_2.session_user) == F.lower(emp_metadata_1.session_user)), how="inner", lsuffix="_l", rsuffix="_r")\
...
def generate_feature_data_set(app_id:int
   ...
   bin_data = binarize_and_stuff_data(auth_logs, work_days, emp_metadata, whitelisted_users, cutoff_days=cutoff_days, target_variable_no_login = target_variable_days_no_login)
...
def run_model_today(session: Session
    ...
    okta_logs = session.table(f"{SNOWPATROL_DB}.{DEVOPS_SH}.{TBL_OKTA_USERS}")\
        .select( 
    ...
    app_logs = session.table(f"{SNOWPATROL_DB}.{DEVOPS_SH}.{TBL_APP_LOGS}")\
        .select(
            F.col('"SESSION_USER"').alias('session_user')
            , F.to_date(F.col('"SNAPSHOT_DATETIME"')).alias('authentication_datetime')
            , F.col('"APP_ID"').alias('app_id')
    ...
    work_days = session.table(f"{SNOWPATROL_DB}.{DEVOPS_SH}.{TBL_WORK_DAYS}")\
    .select(
        F.iff(F.col('"WORK_DAY"'),1,0).alias('work_day')
        , F.col('"SNAPSHOT_DATETIME"').alias('snapshot_datetime')
        )

    emp_metadata = session.table(f"{SNOWPATROL_DB}.{DEVOPS_SH}.{TBL_EMP_METADATA}")\
    ...
    # today = datetime.now().date()
    # max_date = today - timedelta(days=cutoff_days)
    # sql_query = f"SELECT e.session_user, a.snapshot_datetime, DATEDIFF(DAY, a.snapshot_datetime,'{today}') AS days_between FROM snowpatrol_.main.sample_emp_metadata e JOIN snowpatrol_.main.sample_app_logs a ON e.session_user = a.session_user WHERE a.snapshot_datetime <= '{today}'"
    # df = session.sql(sql_query).to_pandas()

    # Convert the result into a Pandas DataFrame

    whitelisted_users = session.table(f"{SNOWPATROL_DB}.{DEVOPS_SH}.{TBL_WHITELISTED_USERS}").select(F.col('"EMAIL"').alias("email"))
    ...
    session.write_pandas(train_user_kpis, f"{TBL_LICENSE_USAGE_FEATURES}",database=f"{SNOWPATROL_DB}",schema=f"{DEVOPS_SH}" ,auto_create_table=True)
    ...
    try:
        ...
        #     session.file.put(model_file, f"@{SNOWPATROL_DB}.{DEVOPS_SH}.{MODELS_STAGE}",overwrite=True, auto_compress=False)
        #     exec_status = {
        #     "status": f"INFO: Model trained as @{SNOWPATROL_DB}.{DEVOPS_SH}.{MODELS_STAGE}/{model_name}",
        #     "run_id": None,
        #     "target_table": None
        #     }
        ...
        #         session
        #         ,udf_name=f"{SNOWPATROL_DB}.{DEVOPS_SH}.udf_predict_login_probability"
        #         ,model=trained_logistic_regression_model
        #         ,predictor_cols=predictor_columns
        #         ,stage_loc=f"@{SNOWPATROL_DB}.{DEVOPS_SH}.{OBJECT_STAGE}"
        ...
        session.write_pandas(test_user_kpis, f"{TBL_LICENSE_REVOCATION_RECOMMENDATION}", database=f"{SNOWPATROL_DB}",schema=f"{DEVOPS_SH}" ,auto_create_table=True, overwrite=False)
        
        exec_status = {
            "status": "SUCCESS",
            "run_id": this_run_id,
            "target_table": f"{SNOWPATROL_DB}.{DEVOPS_SH}.{TBL_LICENSE_REVOCATION_RECOMMENDATION}"
        }
            # license_revocation_recommendation = session.create_dataframe(test_user_kpis)
            # license_revocation_recommendation.write.mode("overwrite").save_as_table(f"{SNOWPATROL_DB}.{DEVOPS_SH}.license_revocation_recommendation")
